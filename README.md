...
The SDK packages (`@mpckit/sdk`, `@mpckit/react`, `@mpckit/core`, and
the `mpckit` Rust crate) ship from this repo; npm + crates.io
publication lands with a follow-up release pass — until then,
consumers can target them as workspace deps or git deps.
...
`v0.1.0` — live on Sui testnet at [api.mpckit.xyz](https://api.mpckit.xyz/v1/network),
mainnet enablement gated on funding the operator's mainnet sponsor
wallet (everything else in the stack already serves both networks).
